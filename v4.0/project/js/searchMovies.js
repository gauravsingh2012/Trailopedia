...
// this passes the hidden id on the div block of the movie to the Watch Movie Trailers Page
...
// this function is called when title of the movie is clicked
// this passes the hidden id on the div block of the movie to the Movie Details Page
...
// this function is called when movie poster image is clicked
// this passes the hidden id on the div block of the movie to the Movie Details Page
...
/// the URL. The input is taken from a text box.

function getMovieTrailer() {

    var searchMovie = $(".search-movie").val();
    var encodedSearchMovie = encodeURIComponent(searchMovie);
    urlString = "https://api.themoviedb.org/3/search/movie?api_key=052ca14131fc2be845560a604c66ffd5&query=" + encodedSearchMovie
    console.log(urlString);
    $.ajax({
        url: "https://api.themoviedb.org/3/search/movie?api_key=052ca14131fc2be845560a604c66ffd5&query=" + encodedSearchMovie,
        crossDomain: true,
        dataType: "jsonp",
        ...
        success: function (data) {
            ...
            if (data.results.length === 0) {
                $(".col-md-12 h1").html("No Movie Found");
                ...
                for (i = 0; i < data.results.length; i++) {
                    fetchData(data.results[i].id, data.results[i].title, data.results[i].poster_path);
                }
            }
...
// fetches movie data from the movie DB API

function fetchData(id, movieName, posterPath) {
    $.ajax({
        url: "https://api.themoviedb.org/3/movie/" + id + "?api_key=052ca14131fc2be845560a604c66ffd5",
        crossDomain: true,
        dataType: "jsonp",
